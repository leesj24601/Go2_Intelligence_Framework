...
*   **Robot Import**: Isaac Sim Nucleus 서버의 공식 Unitree Go2 모델(`go2.usd`)을 가져와 조인트(Joint) 및 충돌체(Collider) 구조를 그대로 상속.
...
*   **Physics Engine**: 로봇의 발끝과 지면이 닿는 접촉부 시뮬레이션을 위해 PhysX 파라미터를 조정. 마찰력 세팅 및 Curriculum 레벨 비활성화를 통해 안정적인 SLAM 스캔이 가능하도록 흔들림을 통제했습니다.
