...
class GuiControllerNode(Node):
    def __init__(self, default_waypoint_file: Path):
        ...
        self.navigator = NavigatorBridge(self.state_bridge, self.waypoint_registry)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self._bind_hold_button(forward_button, 0.2, 0.0, 0.0)
        self._bind_hold_button(back_button, -0.2, 0.0, 0.0)
        self._bind_hold_button(left_button, 0.0, 0.2, 0.0)
        self._bind_hold_button(right_button, 0.0, -0.2, 0.0)
        self._bind_hold_button(turn_left_button, 0.0, 0.0, 0.5)
        self._bind_hold_button(turn_right_button, 0.0, 0.0, -0.5)
    ...
    def _go_to_selected_waypoint(self) -> None:
        ...
        if item is None:
            ...
            return
        try:
            self._node.navigator.go_to_waypoint(item.text())
        except RuntimeError as exc:
            self.feedback_label.setText(f"Feedback: {exc}")
            self._append_log(f"go_to_waypoint blocked: {exc}")
            return
        self.feedback_label.setText(f"Feedback: navigating to {item.text()}")
        self._append_log(f"go_to_waypoint: {item.text()}")
    ...
    def _delete_selected_waypoint(self) -> None:
        ...
        if item is None:
            ...
            return
        deleted = self._node.waypoint_registry.delete_waypoint(item.text())
        if not deleted:
            self.feedback_label.setText("Feedback: failed to delete waypoint")
            self._append_log(f"delete_waypoint failed: {item.text()}")
            return
        self._reload_waypoint_list()
        self.feedback_label.setText(f"Feedback: deleted waypoint {item.text()}")
        self._append_log(f"delete_waypoint: {item.text()}")
    ...
    def _save_current_pose(self) -> None:
        ...
        self._node.waypoint_registry.save_waypoint(
            name=name,
        ...
        self._reload_waypoint_list()
        self.feedback_label.setText(f"Feedback: saved waypoint {name}")
        self._append_log(f"save_waypoint: {name}")
